...
exec bwrap --die-with-parent --unshare-all --ro-bind /usr /usr --ro-bind /etc /etc --proc /proc --dev /dev \
  ...
  /usr/bin/sh -c '
    ...
    sleep 1
    DISPLAY=:1 x11vnc -localhost -rfbport 5998 -forever -shared -nopw -input MB -o /tmp/x11vnc.log >/tmp/vnc-start.log 2>&1 &
    sleep 1
