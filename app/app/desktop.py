...
SOURCE=Path('/home/feed/status.json'); LOG=Path('/tmp/observer.log')
...
class Desktop(Gtk.Window):
 def __init__(self):
  super().__init__(title='Desk Control Room');self.set_decorated(True);self.set_default_size(1280,800);self.set_size_request(1280,800);self.move(0,0)
  ...
  self.monitor,self.monitor_inner,_=frame('SYSTEM MONITOR    •    local display processes',557,330,self);self.canvas.put(self.monitor,704,400)
  ...
  d.pack_end(txt('Remote PC has keyboard and apps   •   this board is read-only','small'),False,False,16);self.canvas.put(dock,0,746)
 ...
 def focus_window(self,button,window):
  # Raise within the fixed desktop by reordering children. The outer window is managed by Xfce.
 ...
 def tick(self):
  ...
  for entry in Path('/proc').iterdir():
   ...
   try:
    ...
    elif 'x11vnc ' in cmd:name='Remote screen relay'
